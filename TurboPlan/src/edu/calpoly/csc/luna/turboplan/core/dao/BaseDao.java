...
public abstract class BaseDao<T extends BaseEntity> {
	...
	protected T getEntityById(Class<T> clazz, Long id) {
		...
		if(clazz.getSimpleName().equals("User")){
			((User)ret).getProfile();
			((User)ret).getProfile().getAddress();
			((User)ret).getProfile().getAvailability();
			((User)ret).getProfile().getGender();
			((User)ret).getProfile().getSkills();
			((User)ret).getCompany();
			((User)ret).getPermission();
			((User)ret).getTasks();
		} else if (clazz.getSimpleName().equals("Task")) {
			((Task)ret).getLocation();
			((Task)ret).getPriority();
			((Task)ret).getRequiredSkill();
			
			for(User u : ((Task)ret).getUsers()){
				u.getProfile();
			}
		}
