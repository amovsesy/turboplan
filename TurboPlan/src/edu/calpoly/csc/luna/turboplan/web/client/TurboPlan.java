...
import edu.calpoly.csc.luna.turboplan.web.client.window.ChangePwWindow;
...
//					LoginWindow.getInstance().show();
			...
			Hyperlink changePass = new Hyperlink("Change Password", "");
			changePass.addClickListener(new ClickListener() {
				public void onClick(Widget sender) {
					Log.debug("Change Password Hyperlink clicked");
					final ChangePwWindow win = new ChangePwWindow();
					win.setModal(true);
					win.show();
				}
			});
			...
			VerticalPanel hPanel_RightSide = new VerticalPanel();
			hPanel_RightSide.add(logout);
			hPanel_RightSide.add(changePass);
			
			
			hPanel.setWidth("100%");
			hPanel.add(new Image("images/product_logo.png"));
//			hPanel.add(logout);
			hPanel.add(hPanel_RightSide);
		...
		private EnumSet<TurboPlanPage> getPageSetFromPermission(Set<String> set) {
			...
			if (set.contains(GwtUser.Permission.MasterSchedule.name())) {
				Log.trace("This user has " + "MasterSchedule" + " persmission");
				ret.add(TurboPlanPage.MasterSchedule);
			}
