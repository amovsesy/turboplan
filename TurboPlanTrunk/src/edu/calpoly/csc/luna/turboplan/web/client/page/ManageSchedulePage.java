...
public class ManageSchedulePage extends BasePage {
	private static final String SIMPLE_CLASSNAME = "ManageSchedulePage";
		...
		ServiceManager.getManageTasksServiceAsync().getTasksForUserById(TurboPlan.loggedInUser.getUserID(),
				new AsyncCallback<Set<GwtTask>>() {
					...
					public void onSuccess(Set<GwtTask> result) {
						...
						cPanel.setBottomToolbar(buildToolbar());

						add((content = cPanel));

						Log.trace("Done constructing ManageSchedulePage");
					}
				});
	...
	private Toolbar buildToolbar() {
		Toolbar bBar = new Toolbar();

		bBar.addItem(new ToolbarFill());
		bBar.addButton(new ToolbarButton("Swap", new ButtonListenerAdapter() {
			public void onClick(Button button, EventObject e) {
			}
		}));
		bBar.addButton(new ToolbarButton("Save", new ButtonListenerAdapter() {
			public void onClick(Button button, EventObject e) {
			}
		}));
